...
| Scope        | Add a Cluster 2 gate before PR for stitch-specific retry/downshift behavior and re-render safety after Photos save. Preserve Cluster 0's visible fallback-note contract and Cluster 1's cache cleanup intent.                                                |
...
| Watchpoints  | Do not touch active bundle identity or TestFlight workflow. Do not treat simulator green as proof of the iPhone bug; use deterministic tests for code paths plus explicit real-device pending notes where hardware behavior cannot be reproduced in simulator. |
