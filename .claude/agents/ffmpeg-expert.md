...
| `public/js/compression.js` | Frontend preset estimates and controls            |
| `lib/probe.js`             | ffprobe wrapper for media analysis                |
| `lib/jobQueue.js`          | HW/SW queue lanes, progress parsing, cancellation |
| `lib/stitch.js`            | Lossless concat and re-encode stitch pipeline     |
...
- Apply current preset-specific ratios from `lib/ffmpeg.js`
...
Legacy aliases accepted by the backend:
...
```
-vf "scale=-2:2160"  # 4K / 2160p
-vf "scale=-2:1440"  # 1440p / 2K
