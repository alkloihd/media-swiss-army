...
| `VideoCompressor/ios/Views/SettingsTabView.swift` | Modify | Add a "Privacy Policy" row that opens `https://alkloihd.github.io/video-compressor-FUCKMETA/privacy/` via `Link` (Safari View Controller / system browser). |
...
> ⚠️ **REQUIRES USER ACTION BEFORE THIS PR CAN MERGE**
>
> The current GitHub Pages URL (`https://alkloihd.github.io/video-compressor-FUCKMETA/privacy/`) embeds the working repo name, which contains profanity. Apple App Review will see this URL in the App Store Connect privacy policy field and may reject for inappropriate language.
>
> **User must choose one BEFORE Codex executes Task 4:**
> - **(a) Rename the GitHub repo** to a clean name (e.g. `media-swiss-army`, `metaclean-ios`). All URLs in this plan need updating to match.
> - **(b) Configure a custom Pages domain** (e.g. `mediaswissarmy.app/privacy`). Update the Settings link target.
> - **(c) Host the privacy policy on a separate non-profane GitHub repo** (e.g. a new `metaclean-privacy` repo with Pages enabled).
>
> Codex: do NOT execute Task 4 until the user resolves this. Surface in the deviation log if you discover this resolved before/during execution.

**Why:** Apple App Store Connect requires a privacy policy URL before submission (App Store Review Guideline 5.1.1). The reference doc `.agents/work-sessions/2026-05-03/reference/PUBLISHING-AND-MONETIZATION.md` Part 8 has the verbatim policy template. We host it on GitHub Pages on the existing repo (`alkloihd/video-compressor-FUCKMETA`) and link to it from the Settings tab so the in-app surface and the App Store Connect entry both point to the same canonical URL.

**Locked URL:** `https://alkloihd.github.io/video-compressor-FUCKMETA/privacy/`. (The reference doc has a stale `alkloihd.github.io/metaclean/privacy` URL — ignore; the locked decision wins, and matches the actual repo name from `AGENTS.md` 16.2.) **NOTE: This URL is blocked pending resolution of the H6 callout above.**
...
<main>
  ...
  <h2>Contact</h2>
  <p>Issues, questions, or feedback: <a href="https://github.com/alkloihd/video-compressor-FUCKMETA/issues">GitHub Issues</a>.</p>
...
```swift
                ...
                Section("About") {
                    Link(
                        // TODO(user): replace with chosen URL per H6 callout above
                        destination: URL(string: "<PRIVACY_POLICY_URL_TBD>")!
                    ) {
                        HStack {
                            Text("Privacy Policy")
                            Spacer()
...
**Important:** The `<PRIVACY_POLICY_URL_TBD>` placeholder above will crash at runtime (force-unwrap of an invalid URL). Do NOT execute Step 2 until the user has resolved the H6 callout at the top of this task and you have a real, profanity-free URL to substitute in.
...
https://alkloihd.github.io/video-compressor-FUCKMETA/privacy/ once
...
- docs/privacy/index.html will serve at https://alkloihd.github.io/video-compressor-FUCKMETA/privacy/ once GitHub Pages is enabled on the repo.
...
- [ ] `https://alkloihd.github.io/video-compressor-FUCKMETA/privacy/` renders the policy after GitHub Pages is enabled (manual user step).
...
- [ ] Privacy policy URL contains no profanity (Apple App Review compliance) — user must resolve H6 callout before this criterion can be met.
...
4. **Open Settings → tap Privacy Policy.** Verify Safari opens to `https://alkloihd.github.io/video-compressor-FUCKMETA/privacy/` and the page renders the dark-mode-aware policy text. (Step requires GitHub Pages enabled — see Notes below.)
...
- **The reference doc `.agents/work-sessions/2026-05-03/reference/PUBLISHING-AND-MONETIZATION.md` Part 8 still references `alkloihd.github.io/metaclean/privacy` — that URL is stale.** The locked URL for this work is `https://alkloihd.github.io/video-compressor-FUCKMETA/privacy/` (matches the actual repo name from `AGENTS.md` 16.2). Use the locked URL throughout; do not "fix" code to match the reference doc.
