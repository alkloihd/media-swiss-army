...
| ☐   | Task 4: Push, PR, CI, merge → TestFlight #1                           | Local pre-PR verification complete: `test_sim` passed `164/164` and `build_sim` succeeded. PR #11 opened; CI green; pre-merge reviewer approved with no blockers. Merge/TestFlight still pending.                                                                               |
