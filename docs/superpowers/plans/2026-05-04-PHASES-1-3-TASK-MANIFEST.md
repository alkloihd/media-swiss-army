...
| ☑   | Task 2: Photos auth gate (TASK-35)               | Added passive `PHPhotoLibrary.authorizationStatus(for: .readWrite)` gating to both `StitchClipFetcher` fetch paths with defaulted test injection. Added 6 auth tests covering `.denied`, `.restricted`, and `.notDetermined` for single and batch lookups. TDD red: missing `authStatusProvider`; green focused tests passed 6/6, full `test_sim` passed 214 total, 213 passed, 1 documented skip. |
...
| 2026-05-04 | 4          | Task 2        | The plan text wanted all three denied-status states across both fetch paths, but its snippet omitted the batch `.restricted` case and referenced `PhotosSaver`'s prompting pattern. Codex added the sixth batch `.restricted` test and used only passive `authorizationStatus(for: .readWrite)`, never `requestAuthorization`, so Stitch sorting cannot trigger a Photos prompt. | Focused auth tests passed 6/6; full `test_sim` passed 214 total, 213 passed, 1 skipped |
