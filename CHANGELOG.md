...
- 2026-05-04 [solo/codex/gpt-5] Cluster 1 cache and still-bake hardening. Still-image stitch bakes are now constant-time one-second movies that are stretched in the composition to the user's selected still duration, while preallocated bake URLs are registered before work starts so cancellation can clean them. `CacheSweeper` now tracks managed temporary wrappers, exposes `sweepOnCancel`, delayed `sweepAfterSave`, and tighter launch sweeping, and the app wires those hooks into compression, stitch, MetaClean, photo compression, Photos save-success, and app-launch paths. Local verification: `test_sim` passed `163/163` and `build_sim` succeeded.
